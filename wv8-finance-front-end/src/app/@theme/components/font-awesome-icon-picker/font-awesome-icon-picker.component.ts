...
@Component({
  selector: "font-awesome-icon-picker",
...
export class FontAwesomeIconPickerComponent implements OnInit {
  ...
  @Input()
  onIconChange = (icon: FontAwesomeIcon) => null;
  ...
  iconsForPage: FontAwesomeIcon[] = [];

  colorSelector = ColorPickerComponent;
  colorSelectorContext: any;
  ...
  select() {
    ...
    switch (this.iconPack) {
      case "far":
        ...
        break;
      case "fas":
        iconsToSearch = this.fasIcons;
        break;
      case "fab":
        iconsToSearch = this.fabIcons;
        break;
      default:
        break;
    }

    iconFound = iconsToSearch.filter(i => i === this.icon).length === 1;
    if (!iconFound) {
      this.toasterService.warning("Icon not found");
      this.cancel();
      return;
    }
  }
  ...
  /*
  Script for icons to be ran on https://fontawesome.com/cheatsheet/free
